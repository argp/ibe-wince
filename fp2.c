/* 
 * NTRG IBE WINCE -- fp2.c
 * Patroklos Argyroudis <argp at domain cs.tcd.ie>
 *
 * Modified version of fp2.c for compilation of the Stanford IBE
 * library on WinCE.
 *
 * $Id: fp2.c,v 1.2 2006/02/13 21:56:37 argp Exp $
 */
...
//inline 
void
zp_mul(mpz_t x, mpz_t a, mpz_t b, mpz_t p)
//x = a * b mod p
{
	mpz_mul(x, a, b);
	mpz_mod(x, x, p);
...
//inline
void
zp_add(mpz_t x, mpz_t a, mpz_t b, mpz_t p)
//x = a + b mod p
{
    mpz_add(x, a, b);
...
//inline
void
zp_neg(mpz_t x, mpz_t b, mpz_t p)
//x = -b mod p
{
    if (mpz_sgn(b)) {
	mpz_sub(x, p, b);
...
//inline
void
zp_sub(mpz_t x, mpz_t a, mpz_t b, mpz_t p)
//x = a - b mod p
{
    if (mpz_cmp(a, b) >= 0) {
	mpz_sub(x, a, b);
